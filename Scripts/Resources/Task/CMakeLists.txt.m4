...
`target_link_libraries'(SampleTask PUBLIC `${OPENAIS_TASK}' -Wl,--whole-archive `${INTERFACES}' -Wl,--no-whole-archive)
...
`configure_file'(`${CMAKE_CURRENT_SOURCE_DIR}/config.json' ConfigFile)
`if(ENABLE_TESTING)'
